...
const CHAT_HELP: &str = "Enter send | Esc back to chats | Ctrl+C quit | Up/Down scroll";
...
fn handle_chat_key(
    ...
    match key.code {
        ...
        KeyCode::Enter => {
            ...
            let prompt = chat.input.trim().to_string();
            if prompt.is_empty() {
                return Ok(false);
            }
...
fn draw_chat(frame: &mut Frame<'_>, app: &App, chat: &ChatState) {
    ...
    let input_x = areas[2].x + 1 + chat.input.chars().count() as u16;
    let input_y = areas[2].y + 1;
